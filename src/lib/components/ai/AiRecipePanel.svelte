<script lang="ts">
  ...
  function buildModifiers(): string {
    ...
    if (optStrict) {
      parts.push(
        " - 엄격 재료 사용: 제공된 재료 목록 외 추가 재료를 절대 사용하지 마세요. "추가추천재료"도 금지입니다. 대체재/옵션/토핑/향신료 등 목록에 없는 모든 재료 금지. 필요 시 레시피 구성을 현재 재료만으로 가능한 방식으로 변경하세요. 사용 가능한 재료가 하나도 없을 경우 항복하세요."
      );
    }
